...
int32 TCPClient::BeginConnect(const FString host, int32 port, std::function<void(FAsyncResultRef)> connectCallback, std::function<void(FAsyncResultRef)> disconnectCallback)
{
    FIPv4Address ip;
    ...
    BeginConnect(endpoint, connectCallback, disconnectCallback);
    return 0;
}

int32 TCPClient::BeginConnect(const FIPv4Endpoint endpoint, std::function<void(FAsyncResultRef)> connectCallback, std::function<void(FAsyncResultRef)> disconnectCallback)
{
...
#pragma endregion
    ...
    ThrdPool->EnqueueJob([=]()
    {
        ...
        auto Ret = MakeShared<TCPAsyncResult, ESPMode::ThreadSafe>(connected, 0);
        connectCallback(Ret);
    }
    );
...
int32 TCPClient::BeginSend(FByteArrayRef& sendBuffPtr, std::function<void(FAsyncResultRef)> callback)
{
#pragma region VALIDATE_REGION
    if (!bConnected)
    {
...
#pragma endregion
    ...
    ThrdPool->EnqueueJob([=]()
    {
        while (!SendingQueue.IsEmpty() && bConnected)
        {
            ...
            TCPPacketHeader header = *reinterpret_cast<TCPPacketHeader*>(SendingMessage->GetData());
            auto Ret = MakeShared<TCPAsyncResult, ESPMode::ThreadSafe>(sendSuccess, processLen, header.Id);
            callback(Ret);
        }
...
int32 TCPClient::BeginRecv(uint8* buffer, int32 bufferSize, std::function<void(FAsyncResultRef)> callback)
{
#pragma region VALIDATE_REGION
    if (!bConnected)
    {
...
#pragma endregion
    ...
    ThrdPool->EnqueueJob([=]()
    {
        ...
        auto Ret = MakeShared<TCPAsyncResult, ESPMode::ThreadSafe >(recvSuccess, byteRead);
        callback(Ret);
    }
    );
    return 0;
...
void TCPClient::Disconnect(const FString& cause, bool shutdownNormally)
{
    ...
    {
        auto Ret = MakeShared<TCPAsyncResult, ESPMode::ThreadSafe >(shutdownNormally, 0);
